...
class LogoutDialog extends ComponentDialog {
    ...
    async interrupt(innerDc) {
        if (innerDc.context.activity.type === ActivityTypes.Message) {
            ...
            {
                await innerDc.context.sendActivity('Hello, checking if current user is valid');
            }
            else if (text === 'help' || text === 'start')
            {
                await innerDc.context.sendActivity('Please try say Hello or Logout');
                return await innerDc.cancelAllDialogs();
            }
            else if (text === 'stop' || text === 'cancel')
            {
                await innerDc.context.sendActivity('Cancel dialog sessions');
                return await innerDc.cancelAllDialogs();
            }
        }
    }
